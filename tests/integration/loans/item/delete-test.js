...
describe('Integration - DELETE /loans/:uuid', function() {
  let controllers;
  let models;
  let server;
  ...
  before('create sinon spies', function() {
    deleteLoanSpy = sinon.spy(controllers.LoanCtrl, 'deleteLoan');
  });

  after('restore sinon spies', function() {
    deleteLoanSpy.restore();
  });
  ...
  it('should return 204 with the correct auth token', async function() {
    ...
    // Validate LoanCtrl.deleteLoan call.
    assert.strictEqual(deleteLoanSpy.callCount, 1);
    const deleteLoanParams = deleteLoanSpy.getCall(0).args[0];
    assert.isOk(deleteLoanParams.auditApiCallUuid);
    assert.strictEqual(deleteLoanParams.loanUuid, user1LoanUuid);
    ...
    const apiCall = await models.Audit.ApiCall.findOne({
      ...
      where: {
        uuid: deleteLoanParams.auditApiCallUuid,
      },
    });
    assert.isOk(apiCall);
