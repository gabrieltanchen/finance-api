...
describe('Integration - DELETE /employers/:uuid', function() {
  let controllers;
  let models;
  let server;
  const testHelper = new TestHelper();

  let deleteEmployerSpy;
  ...
  before('create sinon spies', function() {
    deleteEmployerSpy = sinon.spy(controllers.EmployerCtrl, 'deleteEmployer');
  });

  after('restore sinon spies', function() {
    deleteEmployerSpy.restore();
  });
  ...
  afterEach('reset history for sinon spies', function() {
    deleteEmployerSpy.resetHistory();
  });
  ...
  it('should return 401 with no auth token', async function() {
    const res = await chai.request(server)
      .delete(`/employers/${user1EmployerUuid}`)
    ...
    assert.strictEqual(deleteEmployerSpy.callCount, 0);
  });
  ...
  it('should return 404 with the wrong auth token', async function() {
    ...
    assert.strictEqual(deleteEmployerSpy.callCount, 1);
    const deleteEmployerParams = deleteEmployerSpy.getCall(0).args[0];
    assert.isOk(deleteEmployerParams.auditApiCallUuid);
    assert.strictEqual(deleteEmployerParams.employerUuid, user1EmployerUuid);
  });
  ...
  it('should return 204 with the correct auth token', async function() {
    ...
    // Validate EmployerCtrl.deleteEmployer call.
    assert.strictEqual(deleteEmployerSpy.callCount, 1);
    const deleteEmployerParams = deleteEmployerSpy.getCall(0).args[0];
    assert.isOk(deleteEmployerParams.auditApiCallUuid);
    assert.strictEqual(deleteEmployerParams.employerUuid, user1EmployerUuid);
    ...
    const apiCall = await models.Audit.ApiCall.findOne({
      ...
      where: {
        uuid: deleteEmployerParams.auditApiCallUuid,
      },
    });
    assert.isOk(apiCall);
